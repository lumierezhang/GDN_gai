...
def construct_data(data, feature_map, labels=0):  # （data=（1565,27））
    res = []  # （27列，每列1565条数据）
...
def build_loc_net(struc, all_features, feature_map=[]):
    ...
    for node_name, node_list in struc.items():  # （item为第一行的内容 name：第一个节点的索引 list：第一个节点索引内容（26））
