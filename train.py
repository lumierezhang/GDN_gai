...
def loss_func(y_pred, y_true):  # (128,27) (128,27)
...
def train(model = None, save_path = '', config={},  train_dataloader=None, val_dataloader=None, feature_map={}, test_dataloader=None, test_dataset=None, dataset_name='swat', train_dataset=None):
    ...
    for i_epoch in range(epoch):
        ...
        for x, labels, attack_labels, edge_index in dataloader:  # (128,27,15)(128,27)（128，1）(128,2,702)
            _start = time.time()

            x, labels, edge_index = [item.float().to(device) for item in [x, labels, edge_index]]

            optimizer.zero_grad()
            # print(x.shape,"x")  (torch.size([128,27,15])
            # exit(0)
            out = model(x, edge_index).float().to(device)  # (128,27)
